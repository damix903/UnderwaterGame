...
namespace Input
{
    public class InputReader : IInputProvider, IPlayerActions
    {
        ...
        public void TogglePlayerActionMap(bool isEnabled)
        {
            if (isEnabled) _input.Player.Enable();
            else _input.Player.Disable();
        }

        public void ToggleUIActionMap(bool isEnabled)
        {
            if (isEnabled) _input.UI.Enable();
